...
export class IrDateTimepickerContent {
    ...
    disablePrevMonth: boolean = false;
    disablePrevYear: boolean = false;

    constructor(public dialogRef: MatDialogRef<IrDateTimepickerContent>, @Inject(MAT_DIALOG_DATA) public data: any) {
        ...
        this.todayDateTimeString = 'امروز: ' + this.persianTodayDateTemp[2] + ' ' + this.getPersianMonth(this.persianTodayDateTemp[1]) + ' ' + this.persianTodayDateTemp[0];

        if (this.disableBeforeToday && this.currentYearNumber == parseInt(this.persianTodayDateTemp[0]) && this.currentMonthNumber == parseInt(this.persianTodayDateTemp[1])) {
            this.disablePrevMonth = true;
        }
        
        if (this.disableBeforeToday && this.currentYearNumber == parseInt(this.persianTodayDateTemp[0])) {
            this.disablePrevYear = true;

            for (let i = 0; i < parseInt(this.persianTodayDateTemp[1]) - 1; i++) {
                let b = 0;
                if (i >= 9) {
                    b = 3;
                } else if (i >= 6) {
    ...
    submitTime() {
        if (this.justTime) {
            this.dialogRef.close(`${(this.currentHourNumber.toString().length == 1) ? '0' + this.currentHourNumber : this.currentHourNumber}:${(this.currentMinuteNumber.toString().length == 1) ? '0' + this.currentMinuteNumber : this.currentMinuteNumber}`);
        } else {
            this.selectTime = !this.selectTime;
        }
    }
    ...
    changeDate(day) {
        if (this.showTimer) {
            this.dialogRef.close(`${this.currentYearNumber}/${(this.currentMonthNumber.toString().length == 1) ? '0' + this.currentMonthNumber : this.currentMonthNumber}/${(day.toString().length == 1) ? '0' + day : day} ${(this.currentHourNumber.toString().length == 1) ? '0' + this.currentHourNumber : this.currentHourNumber}:${(this.currentMinuteNumber.toString().length == 1) ? '0' + this.currentMinuteNumber : this.currentMinuteNumber}`);
        } else {
            this.dialogRef.close(`${this.currentYearNumber}/${(this.currentMonthNumber.toString().length == 1) ? '0' + this.currentMonthNumber : this.currentMonthNumber}/${(day.toString().length == 1) ? '0' + day : day}`);
        }
    }
    ...
    selectToday() {
        ...
        if (this.showTimer) {
            this.dialogRef.close(`${this.currentYearNumber}/${(this.currentMonthNumber.toString().length == 1) ? '0' + this.currentMonthNumber : this.currentMonthNumber}/${(this.currentDayNumber.toString().length == 1) ? '0' + this.currentDayNumber : this.currentDayNumber} ${(this.currentHourNumber.toString().length == 1) ? '0' + this.currentHourNumber : this.currentHourNumber}:${(this.currentMinuteNumber.toString().length == 1) ? '0' + this.currentMinuteNumber : this.currentMinuteNumber}`);
        } else {
            this.dialogRef.close(`${this.currentYearNumber}/${(this.currentMonthNumber.toString().length == 1) ? '0' + this.currentMonthNumber : this.currentMonthNumber}/${(this.currentDayNumber.toString().length == 1) ? '0' + this.currentDayNumber : this.currentDayNumber}`);
        }
    }
    ...
    createHTML() {
        ...
        let firstWeekDayNumber = this.getFirstDayOfWeek(this.currentYearNumber, this.currentMonthNumber);
        ...
        for (let i = 1; i <= numberOfDaysInCurrentMonth; i++) {
            ...
            if (this.disableBeforeToday && this.currentYearNumber == parseInt(this.persianTodayDateTemp[0]) && this.currentMonthNumber == parseInt(this.persianTodayDateTemp[1]) && parseInt(this.persianTodayDateTemp[2]) > i) {
    ...
    getInputDateInPersian() {
        ...
        if (this.justTime) { 
            ...
            let gdate = this.jalali.toGregorian(parseInt(jdate[0]), parseInt(jdate[1]), parseInt(jdate[2]));
            date = new Date(gdate.gy, gdate.gm, gdate.gd);
        }
        let persianDate = this.jalali.toJalaali(date.getFullYear(), date.getMonth(), date.getDate());;
    ...
    getTodayCalendarInPersian() {
        ...
        return [persianDate.jy, persianDate.jm, persianDate.jd, (today.getHours().toString().length == 1) ? '0' + today.getHours() : today.getHours().toString(), (today.getMinutes().toString().length == 1) ? '0' + today.getMinutes() : today.getMinutes().toString()];
    }

    getFirstDayOfWeek(persianYear, persianMonth) {
        let gregorianDate = this.jalali.toGregorian(persianYear, persianMonth, 1),
    ...
    decreaseOneYear() {
        ...
        if (this.disableBeforeToday && this.currentYearNumber == parseInt(this.persianTodayDateTemp[0])) {
            ...
            if (this.currentMonthNumber < parseInt(this.persianTodayDateTemp[1])) {
                this.currentMonthNumber = parseInt(this.persianTodayDateTemp[1]);
            }
        }
